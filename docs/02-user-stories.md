...
- **US-3.1** — *(Extended — see Milestone 7 note below)* As a User, I want a calendar view showing events for a month I can browse to (not just the current one), with today's own events also called out regardless of which month I'm viewing, so that I can see what's happening at a glance instead of scanning a flat list.
- **US-3.2** — *(Extended — see Milestone 7 note below)* As a Leader or User, I want a chart view of the top two voted events, so that I can quickly see which ideas have the most community support without the list going on past what's actually decision-relevant.
...
- **Milestone 7 — Cleanup note**: six small, mostly-cosmetic fixes requested after Milestone 6 shipped. Two of them are real behavior changes against existing stories and got full AC/TC treatment: US-3.1 (calendar gains month navigation + an always-visible "today" panel) and US-3.2 (chart truncates to the top two, not the full ranked list) — see `docs/03-acceptance-criteria.md` and `docs/04-test-scenario-inventory.md`. The other four don't get new story/AC/TC entries, the same way Milestone 5/6's small implementation decisions (the `UHeader` toggle, `requireLogin` middleware) didn't — they're documented directly in `docs/07-milestones.md`'s Milestone 7 section instead: the primary highlight color, a post-submission redirect to the calendar, and more variety in the seeded demo data.

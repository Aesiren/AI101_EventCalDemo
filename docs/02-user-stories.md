...
- **US-2.3** — *(Revised — see note below)* As a User, I want to mark myself as a volunteer to help run an event, so that my intent to help (not just attend) is captured accurately, on top of — not instead of — my vote counting toward the event's total.
- **US-2.4** — As a Leader, I want to see the total vote count for each event — including everyone who's volunteered, since volunteering is a stronger form of support than a plain vote — so that I can gauge community demand before deciding on Base support.
...
- **US-3.1** — *(Extended — see Milestone 7 note below)* As a User, I want a calendar view showing events for a month I can browse to (not just the current one), so that I can see what's happening at a glance instead of scanning a flat list.
...
- **US-3.4** — *(Extended — see Milestone 7 note below)* As a User or Leader, I want the home page to show every section of the app as something I can click straight to — with the Calendar and Top Voted entries previewing today's events and the top two voted events right there, not just linking through blind — so that I don't have to already know the site's structure to get around, and can catch what's relevant at a glance before committing to a click.
...
- **Milestone 7 — Cleanup note**: six small, mostly-cosmetic fixes requested after Milestone 6 shipped. One is a real behavior change against an existing story and got full AC/TC treatment: US-3.1 gains calendar month navigation — see `docs/03-acceptance-criteria.md` and `docs/04-test-scenario-inventory.md`. The other five don't get new story/AC/TC entries of their own, the same way Milestone 5/6's small implementation decisions (the `UHeader` toggle, `requireLogin` middleware) didn't — they're documented directly in `docs/07-milestones.md`'s Milestone 7 section instead: the primary highlight color, a post-submission redirect to the calendar, and more variety in the seeded demo data.
  - **Mid-milestone correction**: two of the six items — "show today's events" and "show the top two voted events" — were initially built onto the full `/calendar` and `/votes` pages (as US-3.1/US-3.2 extensions), then reverted after you clarified you'd meant the home dashboard's Calendar and Top Voted *tiles* (US-3.4), not those pages. US-3.1/US-3.2 are back to their pre-Milestone-7 scope (plus calendar's month navigation, which was correctly scoped from the start); the "today" and "top two" behavior now lives in US-3.4 instead. See `docs/07-milestones.md` for the full account.
- **US-2.3 revision note**: originally "volunteer *instead of* vote" — the two were mutually exclusive both as a recorded choice and in what counted toward the vote total. Revised at your request: volunteering still records a distinct `kind` (`'vote'` vs. `'volunteer'` — still one `Interest` record per account/event, so a User still can't be recorded as both at once), but the vote *total* (`getVoteCount`) now counts both kinds — volunteering is a stronger form of support, not an alternative to it, so it shouldn't cost the event a vote. `getMyInterest` (for "already voted"/"already volunteering" UI state) and `getVolunteers` (the Leader-only roster) are unaffected — only the tally changed.
